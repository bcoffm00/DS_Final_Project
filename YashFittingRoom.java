...
public class FittingRoom {
	...
	private static Semaphore semaphore = new Semaphore(MAX_FITTING_ROOMS);
	...
	private static class ClientHandler implements Runnable {
		...
		public void run() {
			...
			try (DataInputStream input = new DataInputStream(new BufferedInputStream(clientSocket.getInputStream()));
				...
				output.writeUTF(
						"Connected to Fitting Room Server. Type 'ENTER' to enter a room, 'EXIT' to leave a room or 'OVER' to disconnect.");
		...
		private void tryAssignRoom() {
			if (!waitingQueue.isEmpty() && semaphore.availablePermits() > 0) {
